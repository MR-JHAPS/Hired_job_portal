...
//-----------THIS CODES ARE FOR SIGNING UP THE COMPANY USER  -----------------------------------------------------------------------------
	...
	//Inserting Company Information:
		public boolean insert_company_information(Connection db_connection, Company_wrapper company_wrapper_object) {
			...
			//Every class Company, Address, City and Country is wrapped inside the Company_wrapper.
			//this one is for the city. We start from city because we need its id for address and we need address id for company.
			try {
				...
				CallableStatement prepare_city = db_connection.prepareCall(sql_query_city);
					...
					//we have created a OUT parameter in Stored procedure of city for obtaining cityID.
...
//-----------GETTING ALL COMPANY INFORMATION USING EMAIL AND PASSWORD ----> FOR SIGN IN VERIFICATION -----------------------------------------
	...
	public List<Company_wrapper> get_company_email_password(Connection db_connection, String company_email,String company_password) {
		...
		try {
			...
			while(rs.next()) {
						...
						//country_row is the object of Country_wrapper that contains address, city and country objects value.
						//I have joined the table company,address,city and country using FK in MYSQL->StoredProcedure.
					Company_wrapper company_wrapper_row = new Company_wrapper(company_row, address_row, city_row, country_row);
...
//INSERTING JOBS / JOB_POSTING:--------------------------------------------------------------------------------------------------------	
	...
	public int insert_city(Connection db_connection, City city, int country_id) throws SQLException {
	    ...
	    //register out parameter means it is given out in stored procedure as "last_insert_id". to use that value in FK in another table.
	    prepare_city.registerOutParameter("sp_city_id", Types.INTEGER);
	    prepare_city.execute();
	...
	public boolean insert_job_information(Connection db_connection, Job_wrapper job_wrapper_object) {
	    try {
	    	//the value of city_id and address_id is passed from method insert city and insert address.
	    	/*In insert city method we are using City class but in this we are using Job_wrapper so we are calling 
	    	 * city object that is wrapped inside the Job_wrapper.
	    	 * */
	        int city_id = insert_city(db_connection, job_wrapper_object.getCity(), job_wrapper_object.getCountry().getId());
